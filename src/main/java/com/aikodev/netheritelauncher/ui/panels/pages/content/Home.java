...
import com.aikodev.netheritelauncher.game.launching.Game;
...
public class Home extends ContentPanel
{
    ...
    {
        // Créer la ComboBox pour les instances
        instanceBox = new ComboBox<>();
        instanceBox.setPromptText("Select an instance");
        instanceBox.getStyleClass().add("instance-box");
        setCanTakeAllSize(instanceBox);
        setLeft(instanceBox);

        // Peupler la ComboBox avec les noms des instances
        List<String> instanceNames = new ArrayList<>();
        for (Instance instance : InstanceManager.getInstances()) {
            instanceNames.add(instance.getName());  // Ajoute le nom de chaque instance
        }
        instanceBox.getItems().addAll(instanceNames);

        // Ajouter les ComboBox à un conteneur
